...
void bodyForce(Body *p, float dt, int n) {
  ...
  int idxWithinGrid = threadIdx.x + blockIdx.x + blockDim.x;
  int stride = gridDim.x * blockDim.x;
  
  for (int i = idxWithinGrid; i < n; i += stride) {
...
int main(const int argc, const char** argv) {
  ...
  const int nIters = 10;  // simulation iterations
   ...
   size_t number_of_blocks = 32 * numberOfSMs;
   ...
   */
    ...
    */
    ...
    integratePos<<<number_of_blocks, threads_per_block>>>(p, dt, nBodies);
    cudaDeviceSynchronize();
